module Resolvers
    class ObtainLikedOrFinishedMediaResolver < BaseResolver
        ...
        def resolve(user_id:, page_num:, limit:, query:, content_type:, type:)
            validate_user
            if type != "liked" && type != "finished"
                raise GraphQL::ExecutionError, "Invalid type"
            end

            if !User.if_visible_to_user(context[:current_user].id.to_i, user_id.to_i)
                raise GraphQL::ExecutionError, "You are not allowed to access this user's liked or finished media"
            end
            ...
            Media.liked_or_finished_media(user_id, content_type, query, page_num, limit, type)
