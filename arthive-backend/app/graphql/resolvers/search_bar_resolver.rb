    ...
    class SearchBarResolver < BaseResolver
        ...
        def resolve(query:, search_type:, search_filter: nil)
...

            {
                ...
                users:   search_type.in?(%w[user all])   ? User.search(query: query, current_user_id: current_user_id) : [],
                reviews: search_type.in?(%w[review all]) ? Review.search(query: query, embedded_query: embedded_query, search_filter: search_filter, current_user_id: current_user_id) : [],
                lists:   search_type.in?(%w[list all])   ? List.search(query: query, embedded_query: embedded_query, search_filter: search_filter, current_user_id: current_user_id) : [],
                threads: search_type.in?(%w[thread all]) ? CommunityThread.search(query: query, embedded_query: embedded_query, search_filter: search_filter, current_user_id: current_user_id) : []
            }
        end
    end
end